// File Name:       day_17/part_1.cpp
// Author:          Samuel Wang (@samuelpswang)
// Purpose:         AoC 2023 Day 17 Part 1 (https://adventofcode.com/2023/day/17)
...
int main() {
    ...
    q.push({0, 0, 0, -1, -1}); // cost, row, column, direction, in-direction count
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            bool is_valid = 0 <= new_row && new_row <= r && 0 <= new_col && new_col <= c && new_count <= 3;
            bool is_forward = (new_dir+2) % 4 != info[2];
            if (is_valid && is_forward) q.push({new_cost, new_row, new_col, new_dir, new_count});
        }
    }
